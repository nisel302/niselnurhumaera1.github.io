...
function deleteTodo(id) {
    if (confirm('apa kamu yakin ingin hapus ini?')) {
...
function deleteAll() {
    if (todos.length === 0) {
        alert('isi tugas terebih dahulu!');
        return;
    }

    if (confirm('apa kamu yakin ingin menghapus semua tugas kamu?')) {
        todos = [];
        saveTodos();
        renderTodos();
    }
...
function renderTodos() {
    ...
    if (filteredTodos.length === 0) {
        todoList.innerHTML = '<div class="no-task">ada tugas hari ini?</div>';
        return;
    }
